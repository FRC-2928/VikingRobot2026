...
public class IntakeGround extends Command {
    ...
    public void execute() {
        ...
        if (this.correction) {
            drivetrain.setControl(drivetrain
                    .driveAndPoint
                    .withVelocityX((robotContainer.joystick1.getLeftX() * speedMultiplier + this.calculateSpeedX())
                            * robotContainer.MaxSpeed)
                    .withVelocityY((robotContainer.joystick1.getLeftY() * speedMultiplier + this.calculateSpeedY())
                            * robotContainer.MaxSpeed));
        }
    }

    public double calculateSpeedX() {
        double output = adjustX.calculate(
                -1 * drivetrain.limelightForward.getTargetHorizontalOffset().in(Units.Degrees), 0);
    ...
    public double calculateSpeedY() {
        double output = adjustY.calculate(
                Math.abs(1 / drivetrain.limelightForward.getTargetHorizontalOffset().in(Units.Degrees)), 0);
